...
var BoundingRect = function() {
  ...
  BoundingRect2.prototype.union = function(other) {
    var x2 = mathMin(other.x, this.x);
    var y2 = mathMin(other.y, this.y);
    if (isFinite(this.x) && isFinite(this.width)) {
      this.width = mathMax(other.x + other.width, this.x + this.width) - x2;
    } else {
      this.width = other.width;
    }
    if (isFinite(this.y) && isFinite(this.height)) {
      this.height = mathMax(other.y + other.height, this.y + this.height) - y2;
    } else {
...
var Handler = function(_super) {
  ...
  Handler2.prototype.findHover = function(x2, y2, exclude) {
    ...
    if (this._pointerSize && !out2.target) {
      ...
      var pointerRect = new BoundingRect_default(x2 - targetSizeHalf, y2 - targetSizeHalf, pointerSize, pointerSize);
...
function cubicRootAt(p0, p1, p2, p3, val, roots2) {
  ...
  if (isAroundZero(A2) && isAroundZero(B2)) {
    ...
    if (isAroundZero(disc)) {
      ...
      var T2 = (2 * A2 * b2 - 3 * a * B2) / (2 * mathSqrt(A2 * A2 * A2));
      var theta = Math.acos(T2) / 3;
      var ASqrt = mathSqrt(A2);
  ...
  Track2.prototype.step = function(target, percent) {
    ...
    if (kfsNum === 1) {
      ...
      if (percent < 0) {
        ...
        var start2 = mathMin5(lastFrame + 1, kfsNum - 1);
        for (frameIdx = start2; frameIdx >= 0; frameIdx--) {
          if (keyframes[frameIdx].percent <= percent) {
            break;
          }
        }
        frameIdx = mathMin5(frameIdx, kfsNum - 2);
      } else {
        ...
        frameIdx = mathMin5(frameIdx - 1, kfsNum - 2);
      }
    ...
    var w2 = interval === 0 ? 1 : mathMin5((percent - frame.percent) / interval, 1);
...
function innerGetBoundingRect(text, font, textAlign, textBaseline) {
  ...
  var rect = new BoundingRect_default(x2, y2, width, height);
  return rect;
}
...
function round(x2, precision, returnStr) {
  if (precision == null) {
    precision = 10;
  }
...
function fromArc(x2, y2, rx, ry, startAngle, endAngle, anticlockwise, min3, max3) {
  ...
  start[0] = mathCos(startAngle) * rx + x2;
  start[1] = mathSin(startAngle) * ry + y2;
  end[0] = mathCos(endAngle) * rx + x2;
  end[1] = mathSin(endAngle) * ry + y2;
  ...
  for (var angle = 0; angle < endAngle; angle += Math.PI / 2) {
    if (angle > startAngle) {
      extremity[0] = mathCos(angle) * rx + x2;
      extremity[1] = mathSin(angle) * ry + y2;
...
var PathProxy = function() {
  ...
  PathProxy2.prototype.lineTo = function(x2, y2) {
    var dx = mathAbs(x2 - this._xi);
    var dy = mathAbs(y2 - this._yi);
    var exceedUnit = dx > this._ux || dy > this._uy;
    this.addData(CMD.L, x2, y2);
  ...
  PathProxy2.prototype.rect = function(x2, y2, w2, h2) {
    ...
    this.addData(CMD.R, x2, y2, w2, h2);
    return this;
  };
      ...
      for (var i = 0; i < len2; ) {
        ...
        switch (cmd) {
          ...
          case CMD.L: {
            x2 = d[i++];
            y2 = d[i++];
            var dx = mathAbs(x2 - xi);
            var dy = mathAbs(y2 - yi);
            if (dx > ux || dy > uy) {
              if (drawPart) {
          ...
          case CMD.R:
            ...
            if (drawPart) {
              ...
              if (accumLength + l > displayedLength) {
                ...
                ctx.moveTo(x2, y2);
                ctx.lineTo(x2 + mathMin3(d_1, width), y2);
                d_1 -= width;
                if (d_1 > 0) {
                  ctx.lineTo(x2 + width, y2 + mathMin3(d_1, height));
                }
                d_1 -= height;
                if (d_1 > 0) {
                  ctx.lineTo(x2 + mathMax3(width - d_1, 0), y2 + height);
                }
                d_1 -= width;
                if (d_1 > 0) {
                  ctx.lineTo(x2, y2 + mathMax3(height - d_1, 0));
                }
                break lo;
              }
              accumLength += l;
...
function containStroke(x0, y0, x1, y1, lineWidth, x2, y2) {
  if (lineWidth === 0) {
    return false;
  }
  var _l = lineWidth;
...
function containStroke2(x0, y0, x1, y1, x2, y2, x3, y3, lineWidth, x4, y4) {
  if (lineWidth === 0) {
    return false;
  }
  var _l = lineWidth;
...
function containStroke3(x0, y0, x1, y1, x2, y2, lineWidth, x3, y3) {
  if (lineWidth === 0) {
    return false;
  }
  var _l = lineWidth;
...
var PI24 = Math.PI * 2;
function containStroke4(cx, cy, r, startAngle, endAngle, anticlockwise, lineWidth, x2, y2) {
  if (lineWidth === 0) {
    return false;
  }
  var _l = lineWidth;
...
function containPath(path, lineWidth, isStroke, x2, y2) {
  ...
  for (var i = 0; i < len2; ) {
    ...
    switch (cmd) {
      ...
      case CMD2.L:
        if (isStroke) {
          if (containStroke(xi, yi, data[i], data[i + 1], lineWidth, x2, y2)) {
            return true;
          }
        } else {
      ...
      case CMD2.C:
        if (isStroke) {
          if (containStroke2(xi, yi, data[i++], data[i++], data[i++], data[i++], data[i], data[i + 1], lineWidth, x2, y2)) {
            return true;
          }
        } else {
      ...
      case CMD2.Q:
        if (isStroke) {
          if (containStroke3(xi, yi, data[i++], data[i++], data[i], data[i + 1], lineWidth, x2, y2)) {
            return true;
          }
        } else {
      ...
      case CMD2.A:
        ...
        if (isStroke) {
          if (containStroke4(cx, cy, ry, theta, theta + dTheta, anticlockwise, lineWidth, _x, y2)) {
            return true;
          }
        } else {
      ...
      case CMD2.R:
        ...
        y1 = y0 + height;
        if (isStroke) {
          if (containStroke(x0, y0, x1, y0, lineWidth, x2, y2) || containStroke(x1, y0, x1, y1, lineWidth, x2, y2) || containStroke(x1, y1, x0, y1, lineWidth, x2, y2) || containStroke(x0, y1, x0, y0, lineWidth, x2, y2)) {
            return true;
          }
        } else {
      ...
      case CMD2.Z:
        if (isStroke) {
          if (containStroke(xi, yi, x0, y0, lineWidth, x2, y2)) {
            return true;
          }
        } else {
          w2 += windingLine(xi, yi, x0, y0, x2, y2);
...
function containStroke5(pathProxy, lineWidth, x2, y2) {
    ...
    if (rect.contain(x2, y2)) {
      ...
      if (this.hasStroke()) {
        ...
        if (lineScale > 1e-10) {
          ...
          if (containStroke5(pathProxy, lineWidth / lineScale, x2, y2)) {
            return true;
          }
        }
      }
      ...
      if (ocrMax > e || icrMax > e) {
        ...
        if (arc < PI4) {
          ...
          if (it_1) {
            ...
            var b2 = mathSqrt4(it_1[0] * it_1[0] + it_1[1] * it_1[1]);
            limitedOcrMax = mathMin4(ocrMax, (radius - b2) / (a + 1));
            limitedIcrMax = mathMin4(icrMax, (innerRadius - b2) / (a - 1));
          }
        }
      }
    }
...
var Ring = function(_super) {
  ...
  Ring2.prototype.buildPath = function(ctx, shape) {
    ...
    ctx.arc(x2, y2, shape.r, 0, PI28, false);
    ctx.moveTo(x2 + shape.r0, y2);
    ctx.arc(x2, y2, shape.r0, 0, PI28, true);
  };
  return Ring2;
}(Path_default);
    ...
    DataStore2.prototype.minmaxDownSample = function(valueDimension, rate) {
      ...
      for (var i = 0; i < len2; i += frameSize) {
        ...
        for (var k2 = 0; k2 < thisFrameSize; k2++) {
          var rawIndex = this.getRawIndex(i + k2);
...
function createSymbol(symbolType, x2, y2, w2, h2, color, keepAspect) {
  ...
  if (symbolType.indexOf("image://") === 0) {
    symbolPath = makeImage(symbolType.slice(8), new BoundingRect_default(x2, y2, w2, h2), keepAspect ? "center" : "cover");
  } else if (symbolType.indexOf("path://") === 0) {
    symbolPath = makePath(symbolType.slice(7), {}, new BoundingRect_default(x2, y2, w2, h2), keepAspect ? "center" : "cover");
  } else {
  ...
  function setPatternnSource(pattern2) {
    ...
    function brushDecal() {
      ...
      function brushSymbol(x3, y3, width2, height2, symbolType) {
        ...
        var symbol = createSymbol(symbolType, x3 * scale4, y3 * scale4, width2 * scale4, height2 * scale4, decalOpt.color, decalOpt.symbolKeepAspect);
        if (isSVG) {
...
var EPSILON4 = 1e-8;
function isAroundEqual2(a, b2) {
  return Math.abs(a - b2) < EPSILON4;
}
function contain3(points2, x2, y2) {
  var w2 = 0;
  var p = points2[0];
  if (!p) {
    return false;
  ...
  for (var i = 1; i < points2.length; i++) {
    var p2 = points2[i];
    w2 += windingLine(p[0], p[1], p2[0], p2[1], x2, y2);
    p = p2;
  }
  var p0 = points2[0];
  if (!isAroundEqual2(p[0], p0[0]) || !isAroundEqual2(p[1], p0[1])) {
    w2 += windingLine(p[0], p[1], p0[0], p0[1], x2, y2);
  }
  return w2 !== 0;
}
...
var GeoJSONRegion = (
  ...
  function(_super) {
        ...
        for (var i = 0, len2 = geometries.length; i < len2; i++) {
          ...
          if (contain3(exterior, coord[0], coord[1])) {
            for (var k2 = 0; k2 < (interiors ? interiors.length : 0); k2++) {
              if (contain3(interiors[k2], coord[0], coord[1])) {
                continue loopGeo;
              }
            }
            return true;
    ...
    GeoJSONRegion2.prototype.transformTo = function(x2, y2, width, height) {
      ...
      var target = new BoundingRect_default(x2, y2, width, height);
  ...
  for (var i = 0; i < data.length; ) {
    ...
    switch (cmd) {
      ...
      case CMD4.R:
        ...
        var width = data[i++];
        var height = data[i++];
        d = projectPointToRect(x0, y0, width, height, x2, y2, tmpPt);
        break;
      case CMD4.Z:
        d = projectPointToLine(xi, yi, x0, y0, x2, y2, tmpPt, true);
        xi = x0;
        yi = y0;
        break;
    }
...
function handleScroll() {
  ...
  this.__resize_raf__ = requestAnimationFrame2(function() {
  ...
  return b = Object.assign || function(e2) {
    for (var t, n = 1, r = arguments.length; n < r; n++)
      for (var i in t = arguments[n])
        Object.prototype.hasOwnProperty.call(t, i) && (e2[i] = t[i]);
    return e2;
  ...
  function u() {
    ...
    if (e3.setAttribute("type", "text/css"), t.attributes)
      for (var n2 = Object.keys(t.attributes), i2 = 0; i2 < n2.length; i2++)
        e3.setAttribute(n2[i2], t.attributes[n2[i2]]);
...
var H = defineComponent({ name: "echarts", props: b(b({ option: Object, theme: { type: [Object, String] }, initOptions: Object, updateOptions: Object, group: String, manualUpdate: Boolean }, _), L), emits: {}, inheritAttrs: false, setup: function(t, n) {
  ...
  function G(e2) {
    if (v.value) {
      var n2 = y2.value = init2(v.value, F.value, N.value);
      t.group && (n2.group = t.group), Object.keys(Z).forEach(function(e3) {
        var t2 = Z[e3];
        if (t2) {
  ...
  W ? Object.keys(W).forEach(function(e2) {
    ...
    if (0 !== t2.indexOf("native:"))
      "Once" === t2.substring(t2.length - 4) && (t2 = "~".concat(t2.substring(0, t2.length - 4))), Z[t2] = a[e2];
    else {
  ...
  watch(S2, function(n2) {
    "function" == typeof K && (K(), K = null), n2 || (K = watch(function() {
      ...
      e2 && (y2.value ? y2.value.setOption(e2, b({ notMerge: e2 !== t2 }, $.value)) : G());
    }, { deep: true }));
  ...
  return function(e2, t2, n2) {
    ...
    var i = null;
    watch([r, t2, n2], function(e2, t3, n3) {
      var r2 = e2[0], o = e2[1], a2 = e2[2];
      if (r2 && o && a2) {
  ...
  }), b({ chart: y2, root: u, inner: v, setOption: function(e2, n2) {
    t.manualUpdate && (_2.value = e2), y2.value ? y2.value.setOption(e2, n2 || {}) : G(e2);
  }, nonEventAttrs: q, nativeListeners: I }, Q);
}, render: function() {
  var e2 = Vue2 ? { attrs: this.nonEventAttrs, on: this.nativeListeners } : b(b({}, this.nonEventAttrs), this.nativeListeners);
